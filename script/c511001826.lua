--Memories of Courage
function c511001826.initial_effect(c)
	--Activate
	...
	e1:SetCondition(c511001826.condition)
	e1:SetTarget(c511001826.target)
	e1:SetOperation(c511001826.activate)
	c:RegisterEffect(e1)
	if not c511001826.global_check then
		c511001826.global_check=true
		local ge1=Effect.CreateEffect(c)
		ge1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge1:SetCode(EVENT_BATTLE_DAMAGE)
		ge1:SetOperation(c511001826.desop)
		Duel.RegisterEffect(ge1,0)
		local ge2=Effect.CreateEffect(c)
		ge2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge2:SetCode(EVENT_BATTLED)
		ge2:SetOperation(c511001826.desop2)
		Duel.RegisterEffect(ge2,0)
	end
end
function c511001826.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentPhase()~=PHASE_DAMAGE or not Duel.IsDamageCalculated()
end
function c511001826.filter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x48) and c:IsType(TYPE_XYZ) and c:GetOverlayCount()==0 
		and c:GetFlagEffect(511001825+tp)>0
end
function c511001826.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c511001826.filter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c511001826.filter,tp,LOCATION_MZONE,0,1,nil,tp) 
		and Duel.IsExistingMatchingCard(Card.IsSetCard,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,nil,0x48) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	Duel.SelectTarget(tp,c511001826.filter,tp,LOCATION_MZONE,0,1,1,nil,tp)
end
function c511001826.activate(e,tp,eg,ep,ev,re,r,rp)
...
function c511001826.desop(e,tp,eg,ep,ev,re,r,rp)
...
function c511001826.desop2(e,tp,eg,ep,ev,re,r,rp)
	local dc=Duel.GetAttackTarget()
	if not dc then return end
	local bc=dc:GetBattleTarget()
	if not bc:IsSetCard(0x48) or not bc:IsType(TYPE_XYZ) or not dc:IsSetCard(0x48) or not dc:IsType(TYPE_XYZ) 
