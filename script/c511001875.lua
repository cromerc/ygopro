--Overlay Hunt
function c511001875.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DISABLE)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_ATTACK_ANNOUNCE)
	e1:SetCondition(c511001875.condition)
	e1:SetTarget(c511001875.target)
	e1:SetOperation(c511001875.activate)
	c:RegisterEffect(e1)
end
function c511001875.condition(e,tp,eg,ep,ev,re,r,rp)
	local a=Duel.GetAttacker()
	local d=Duel.GetAttackTarget()
	return a:IsType(TYPE_XYZ) and a:IsControler(1-tp) and d and d:IsFaceup() 
		and d:IsType(TYPE_XYZ) and d:IsControler(tp)
end
function c511001875.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511001875.activate(e,tp,eg,ep,ev,re,r,rp)
	local a=Duel.GetAttacker()
	local d=Duel.GetAttackTarget()
