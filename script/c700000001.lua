...
function c700000001.filter(c)
	return c:IsSetCard(0x33) and c:GetLevel()>0
end
function c700000001.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c700000001.filter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	Duel.SelectTarget(tp,c700000001.filter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
end
function c700000001.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) and c:IsFaceup() and tc:IsFaceup() then
		local e1=Effect.CreateEffect(c)
