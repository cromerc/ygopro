...
function c511001249.filter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x54)
		and Duel.IsPlayerCanSpecialSummonMonster(tp,511001249,0,0x11,c:GetAttack(),c:GetDefense(),c:GetLevel(),c:GetRace(),c:GetAttribute())
end
...
function c511001249.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,511001249,0,0x11,tc:GetAttack(),tc:GetDefense(),tc:GetLevel(),tc:GetRace(),tc:GetAttribute()) then return end
