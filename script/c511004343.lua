--Spirit Battle
function c511004343.initial_effect(c)
	--active
	...
	e1:SetTarget(c511004343.target)
	e1:SetOperation(c511004343.operation)
	c:RegisterEffect(e1)
end
function c511004343.filter(c,e,tp,tid)
	return bit.band(c:GetReason(),0x21)==0x21 and c:GetTurnID()==tid and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c511004343.target(e,tp,eg,ev,ep,re,r,rp,chk)
	local tid=Duel.GetTurnCount()
	if chk==0 then return Duel.IsExistingTarget(c511004343.filter,tp,LOCATION_GRAVE,0,1,nil,e,tp,tid) and Duel.IsExistingTarget(c511004343.filter,tp,0,LOCATION_GRAVE,1,nil,e,tp,tid) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0 end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local tg1=Duel.SelectTarget(tp,c511004343.filter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp,tid)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local tg2=Duel.SelectTarget(1-tp,c511004343.filter,tp,0,LOCATION_GRAVE,1,1,nil,e,tp,tid)
...
function c511004343.operation(e,tp,eg,ev,ep,re,r,rp,chk)
