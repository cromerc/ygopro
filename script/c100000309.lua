...
function c100000309.discon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.GetAttacker():IsSetCard(0x3013)
		and ep~=tp and re:GetOwner():IsType(TYPE_TRAP) and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
...
function c100000309.piercecon(e,tp,eg,ep,ev,re,r,rp)
	...
	return d and a:IsControler(tp) and d:IsDefencePos() and a:IsSetCard(0x3013)
end
