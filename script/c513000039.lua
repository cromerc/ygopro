--時械神メタイオン
function c513000039.initial_effect(c)
	--no damage
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_CONTINUOUS+EFFECT_TYPE_SINGLE)
	e1:SetCode(EVENT_PRE_BATTLE_DAMAGE)
	e1:SetOperation(c513000039.damop)
	c:RegisterEffect(e1)
	--indes
	...
	e6:SetCondition(c513000039.thcon)
	e6:SetTarget(c513000039.thtg)
	e6:SetOperation(c513000039.thop)
	c:RegisterEffect(e6)
	--to deck
	...
	e7:SetCondition(c513000039.tdcon)
	e7:SetTarget(c513000039.tdtg)
	e7:SetOperation(c513000039.tdop)
	c:RegisterEffect(e7)
	--disable summon
	...
	e9:SetTarget(c513000039.sumlimit)
	c:RegisterEffect(e9)
end
...
function c513000039.tdcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()==tp
end
function c513000039.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,e:GetHandler(),1,0,0)
end
function c513000039.tdop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and c:IsFaceup() and c:IsAbleToDeck() then
		Duel.SendtoDeck(c,nil,2,REASON_EFFECT)
	end
end
function c513000039.sumlimit(e,c,sump,sumtype,sumpos,targetp,se)
