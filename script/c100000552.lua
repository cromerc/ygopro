...
function c100000552.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x1f)
end
function c100000552.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c100000552.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c100000552.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c100000552.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
