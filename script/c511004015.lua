...
function c511004015.cost(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsAbleToHand() end
	if chk==0 then return Duel.CheckReleaseGroup(tp,nil,1,nil)
		or Duel.CheckReleaseGroup(tp,c511004015.filter2,1,nil,tp,e) end
	local sel=0
	if Duel.CheckReleaseGroup(tp,nil,1,nil) then
		if Duel.IsExistingTarget(c511004015.filter,tp,0,LOCATION_ONFIELD,1,e:GetHandler()) then
			sel=sel+2
		else
			sel=sel+1
		end
	end
	if Duel.CheckReleaseGroup(tp,c511004015.filter2,1,nil,tp,e) then
		sel=sel+3
	end
	local op=0
	...
	elseif op==2 then
		sg=Duel.SelectReleaseGroup(tp,c511004015.filter2,1,1,nil,tp,e)
	end
	if op==0 then
		e:SetCategory(CATEGORY_DAMAGE)
		e:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
		e:SetTarget(c511004015.target)
		e:SetOperation(c511004015.operation)
	...
	elseif op==1 then
		...
		e:SetTarget(c511004015.target2)
		e:SetOperation(c511004015.operation2)
		e:SetLabelObject(sg:GetFirst())
	elseif op==2 then
		e:SetCategory(CATEGORY_DESTROY+CATEGORY_DAMAGE)
		e:SetProperty(EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_PLAYER_TARGET)
		e:SetTarget(c511004015.target3)
		e:SetOperation(c511004015.operation3)
...
function c511004015.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(1-tp)
	Duel.SetTargetParam(e:GetLabel())
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,e:GetLabel())
end
function c511004015.operation(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Damage(p,d,REASON_EFFECT)
end
function c511004015.filter(c)
	return c:IsDestructable() and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsFaceup()
end
function c511004015.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(e:GetLabelObject():GetOwner())
	Duel.SetTargetParam(e:GetLabelObject():GetAttack()/2)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c511004015.filter,tp,0,LOCATION_ONFIELD,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,e:GetLabelObject():GetAttack()/2)
end
function c511004015.operation2(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
...
function c511004015.filter2(c,tp,e)
...
function c511004015.target3(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c511004015.filter3,tp,0,LOCATION_MZONE,1,1,nil,e:GetLabel())
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,tc:GetTextAttack()/2)
end
function c511004015.operation3(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) then
		if Duel.Destroy(tc,REASON_EFFECT)~=0 then
