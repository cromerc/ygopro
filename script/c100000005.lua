...
function c100000005.initial_effect(c)
	...
	c:RegisterEffect(e4)
	local e5=e3:Clone()
	e5:SetCondition(c100000005.dacon)
	...
	e6:SetCondition(c100000005.con)
	e6:SetTarget(c100000005.tg)
	e6:SetOperation(c100000005.op)
	c:RegisterEffect(e6)
	--to deck
...
function c100000005.dacon(e)
	return e:GetHandler():IsAttackPos()
end
function c100000005.con(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetBattledGroupCount()>0 or e:GetHandler():GetAttackedCount()>0
end
function c100000005.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:GetControler()~=tp and chkc:IsOnField() and chkc:IsAbleToDeck() end
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,Card.IsAbleToDeck,tp,0,LOCATION_ONFIELD,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,g,1,0,0)
end
function c100000005.op(e,tp,eg,ep,ev,re,r,rp)
