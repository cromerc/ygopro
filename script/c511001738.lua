...
function c511001738.initial_effect(c)
	--Activate
	...
	e1:SetCondition(c511001738.condition1)
	e1:SetTarget(c511001738.target)
	e1:SetOperation(c511001738.activate)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	e2:SetDescription(aux.Stringid(10000080,1))
	e2:SetCondition(c511001738.condition2)
	c:RegisterEffect(e2)
end
function c511001738.condition1(e,tp,eg,ep,ev,re,r,rp)
	return ep==tp and bit.band(r,REASON_EFFECT)~=0
end
function c511001738.condition2(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp and bit.band(r,REASON_EFFECT)~=0
end
function c511001738.filter(c,e,tp)
	return c:GetLevel()==5 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c511001738.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511001738.activate(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsPlayerCanSpecialSummonMonster(ep,0,0,TYPE_MONSTER,0,0,5,0,0) then return end
	local g=Duel.GetMatchingGroup(c511001738.filter,ep,LOCATION_HAND,0,nil,e,ep)
	if Duel.GetLocationCount(ep,LOCATION_MZONE)>0 and g:GetCount()>0 and Duel.SelectYesNo(ep,aux.Stringid(20758643,1)) then
