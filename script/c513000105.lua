--Magical Alms
function c513000105.initial_effect(c)
	--activation
	...
	e1:SetTarget(c513000105.target)
	e1:SetOperation(c513000105.activate)
	c:RegisterEffect(e1)
	--adjust
	...
	e2:SetCountLimit(1)
	e2:SetOperation(c513000105.gop)
	c:RegisterEffect(e2)
end
function c513000105.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(1000)
	Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,1000)
end
function c513000105.activate(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Recover(p,d,REASON_EFFECT)
end
function c513000105.gop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetLabel(0)
	e1:SetOperation(c513000105.regop)
	...
	e2:SetCondition(c513000105.damcon)
	e2:SetOperation(c513000105.damop)
	e2:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
	c:RegisterEffect(e2)
end
function c513000105.damcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetLabel()>e:GetLabelObject():GetLabel()
end
function c513000105.damop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	Duel.Damage(Duel.GetTurnPlayer(),e:GetLabel()*1000,REASON_EFFECT)
end
function c513000105.regop(e,tp,eg,ep,ev,re,r,rp)
