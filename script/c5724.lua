--真紅眼の遡刻竜
function c5724.initial_effect(c)
	--spsummon
	...
	e1:SetCondition(c5724.spcon)
	e1:SetTarget(c5724.sptg)
	e1:SetOperation(c5724.spop)
	c:RegisterEffect(e1)
	--extra summon
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCost(c5724.sumcost)
	e2:SetOperation(c5724.sumop)
	c:RegisterEffect(e2)
end
function c5724.cfilter(c,tp)
...
function c5724.spcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c5724.cfilter,1,nil,tp)
end
function c5724.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanSpecialSummonCount(tp,2)
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c5724.spfilter(c,e,tp)
	return c:IsCanBeSpecialSummoned(e,0,tp,false,false,c:GetPreviousPosition())
end
function c5724.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c5724.sumcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsReleasable() end
	Duel.Release(e:GetHandler(),REASON_COST)
end
function c5724.sumop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetFlagEffect(tp,5724)~=0 then return end
	...
	Duel.RegisterFlagEffect(tp,5724,RESET_PHASE+PHASE_END,0,1)
end
