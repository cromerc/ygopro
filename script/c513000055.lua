--二重魔法
function c513000055.initial_effect(c)
	--copy spell
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetTarget(c513000055.target)
	e1:SetOperation(c513000055.operation)
	c:RegisterEffect(e1)
end
function c513000055.filter(c,e,tp,eg,ep,ev,re,r,rp,tid)
...
function c513000055.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		if (b and ft>1) or (not b and ft>0) then
			return Duel.IsExistingTarget(c513000055.filter,tp,0,LOCATION_GRAVE,1,e:GetHandler(),e,tp,eg,ep,ev,re,r,rp,tid)
		else
			return false
		end
	end
	...
	Duel.SelectTarget(tp,c513000055.filter,tp,0,LOCATION_GRAVE,1,ft,nil,e,tp,eg,ep,ev,re,r,rp,tid)
end
function c513000055.operation(e,tp,eg,ep,ev,re,r,rp)
