...
function c511000135.initial_effect(c)
	...
	e1:SetTarget(c511000135.target)
	e1:SetOperation(c511000135.operation)
	c:RegisterEffect(e1)
	--Equip limit
	...
	e2:SetValue(c511000135.eqlimit)
	c:RegisterEffect(e2)
	--damage
...
function c511000135.eqlimit(e,c)
	return c:IsFaceup()
end
function c511000135.filter(c)
	return c:IsFaceup()
end
function c511000135.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c511000135.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c511000135.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c511000135.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,e:GetHandler(),1,0,0)
end
function c511000135.operation(e,tp,eg,ep,ev,re,r,rp)
