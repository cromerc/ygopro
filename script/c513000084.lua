--エコール・ド・ゾーン
function c513000084.initial_effect(c)
	--Activate
	...
	e2:SetTarget(c513000084.destg)
	e2:SetOperation(c513000084.desop)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e5:SetTarget(c513000084.destg2)
	e5:SetOperation(c513000084.desop2)
	c:RegisterEffect(e5)
	local e6=e5:Clone()
...
function c513000084.destg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c513000084.desop(e,tp,eg,ep,ev,re,r,rp)
...
function c513000084.filter(c,re,e)
	return c:IsFaceup() and (not re or re:GetOwner()~=e:GetOwner())
end
function c513000084.destg2(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=eg:GetFirst()
	if chk==0 then return eg:IsExists(c513000084.filter,1,nil,re,e) end
	local ct=eg:GetCount()
	Duel.SetOperationInfo(0,CATEGORY_DISABLE_SUMMON,eg,ct,0,0)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,eg,ct,0,0)
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,ct,0,0)
...
function c513000084.desop2(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	Duel.NegateSummon(eg)
	Duel.Destroy(eg,REASON_EFFECT)
	local tc=eg:GetFirst()
