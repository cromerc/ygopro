...
function c511009335.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCodeFun(c,64751286,c511009335.ffilter,1,true,true)
	...
	e3:SetCondition(c511009335.atkcond)
	e3:SetTarget(c511009335.atktg)
	e3:SetOperation(c511009335.atkop)
	c:RegisterEffect(e3)
	...
	e8:SetCondition(c511009335.atkcond2)
	e8:SetTarget(c511009335.atktg2)
	e8:SetOperation(c511009335.atkop2)
	c:RegisterEffect(e8)
end

function c511009335.ffilter(c)
	return c:IsSetCard(0x52) and c:IsAttribute(ATTRIBUTE_DARK)
end
...
-----atk down
function c511009335.atkcond(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=eg:GetFirst()
	return tc:GetPreviousControler()==tp and  tc:IsCode(511009337) and tc:IsReason(REASON_BATTLE) 
end
function c511009335.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	local tg=eg:GetFirst():GetReasonCard()
	if chk==0 then return tg:IsOnField() end
	Duel.SetTargetCard(tg)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,tg,1,0,0)
end
function c511009335.atkop(e,tp,eg,ep,ev,re,r,rp)
...
------atk down2
function c511009335.cfilter(c,tp)
	return c:IsCode(511009337) and c:IsReason(REASON_DESTROY) and c:GetReasonPlayer()~=tp
end
function c511009335.atkcond2(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c511009335.cfilter,1,nil,tp) and bit.band(r,REASON_EFFECT)~=0  and re:IsActiveType(TYPE_MONSTER) and re:GetControler()==1-tp
end
function c511009335.atktg2(e,tp,eg,ep,ev,re,r,rp,chk)
	local tg=re:GetHandler()
	if chk==0 then return tg:IsOnField() end
	Duel.SetTargetCard(tg)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,tg,1,0,0)
end
function c511009335.atkop2(e,tp,eg,ep,ev,re,r,rp)
local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) and tc:IsFaceup() and not tc:IsImmuneToEffect(e) then
		local e1=Effect.CreateEffect(e:GetHandler())
		e1:SetType(EFFECT_TYPE_SINGLE)
		...
		e1:SetReset(RESET_EVENT+0x1fe0000)
		tc:RegisterEffect(e1)
		Duel.Recover(e:GetHandler():GetControler(),800,REASON_EFFECT)
	end
end