...
function c110000109.initial_effect(c)
	...
	c:RegisterEffect(e5)
	if not c110000109.global_check then
		c110000109.global_check=true
		if Duel.GetFlagEffect(0,110000104)==0 and Duel.GetFlagEffect(1,110000104)==0 then
			...
			ge1:SetCondition(c110000109.atkcon)
			ge1:SetTarget(c110000109.atktg)
			...
			ge2:SetOperation(c110000109.checkop)
			...
			ge3:SetCondition(c110000109.atcon)
			ge3:SetOperation(c110000109.atop)
...
function c110000109.atcon(e,tp,eg,ep,ev,re,r,rp)
...
function c110000109.atop(e,tp,eg,ep,ev,re,r,rp)
	local atg=Duel.GetAttacker():GetAttackableTarget()
	local d=Duel.GetAttackTarget()
	if atg:IsExists(c110000109.filter,1,d) and Duel.SelectYesNo(tp,aux.Stringid(21558682,0)) then
		local g=atg:FilterSelect(tp,c110000109.filter,1,1,d)
