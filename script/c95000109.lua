...
function c95000109.initial_effect(c)
	...
	e2:SetOperation(c95000109.repop)
	c:RegisterEffect(e2)		
	--unaffectable
	...
	e8:SetCondition(c95000109.condition)
	e8:SetTarget(c95000109.Acttarget)
	e8:SetOperation(c95000109.operation)
	c:RegisterEffect(e8)
...
--speed Duel Filter
function c95000109.SDfilter(c)
	return c:GetCode()==511004001
end
--vanilla mode filter
function c95000109.Vfilter(c)
...
function c95000109.op(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.IsExistingMatchingCard(c95000109.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) and Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,nil):GetCount()<20 then
	Duel.Win(1-tp,0x55)
	end
	if Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,e:GetHandler()):GetCount()<40 and not Duel.IsExistingMatchingCard(c95000109.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
	Duel.Win(1-tp,0x55)
	end
	...
	if tc==nil then
		...
		-- if Duel.IsExistingMatchingCard(c95000109.Vfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
			-- c95000109.tableAction.push(95000200)
		-- end
	else
...
Duel.RegisterFlagEffect(tp,95000109,RESET_PHASE+PHASE_END,0,1)
end
...
function c95000109.condition(e,tp,eg,ep,ev,re,r,rp)
	...
	and Duel.GetFlagEffect(e:GetHandlerPlayer(),95000109)==0
...
function c95000109.repop(e)
	local c=e:GetHandler()
		...
		e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		e1:SetLabelObject(c)
		e1:SetOperation(c95000109.returnop)
		Duel.RegisterEffect(e1,0)
		c:RegisterFlagEffect(900000007,0,0,1)
	end
	Duel.SendtoDeck(c,nil,-2,REASON_RULE)
end
function c95000109.returnop(e)
	...
	if fc and fc:GetFlagEffect(195000109)==0 then
	--action card get
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(95000109,0))
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetRange(LOCATION_SZONE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCondition(c95000109.condition)
	e1:SetTarget(c95000109.Acttarget)
...
function c95000109.sumcondition(e,tp,eg,ep,ev,re,r,rp)
	...
	return eg:IsExists(c95000109.cfilter,1,nil,tp) and (f1:GetFlagEffect(95000109)==0) and (f2:GetFlagEffect(95000109)==0)
end
