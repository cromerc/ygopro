--ＳｐTakeover
function c511000978.initial_effect(c)
	--Activate
	...
	e1:SetCondition(c511000978.con)
	e1:SetTarget(c511000978.target)
	e1:SetOperation(c511000978.activate)
	c:RegisterEffect(e1)
end
function c511000978.con(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFieldCard(tp,LOCATION_SZONE,5)
	return tc and tc:GetCounter(0x91)>3
end
function c511000978.filter(c)
	return c:IsFaceup() and c:GetCounter(0)~=0 and c:IsDestructable()
end
function c511000978.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and c511000978.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c511000978.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c511000978.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
