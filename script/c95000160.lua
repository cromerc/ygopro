--Action Field - Closed Forest
function c95000160.initial_effect(c)
--Activate	
	...
	e1:SetOperation(c95000160.op)
	c:RegisterEffect(e1)
	--redirect
	local e2=Effect.CreateEffect(c)
	e2:SetCode(EVENT_LEAVE_FIELD)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e2:SetOperation(c95000160.repop)
	c:RegisterEffect(e2)		
	--unaffectable
	...
	e8:SetCondition(c95000160.condition)
...
--speed Duel Filter
function c95000160.SDfilter(c)
	return c:GetCode()==511004001
end
--vanilla mode filter
function c95000160.Vfilter(c)
	return c:GetCode()==511004002
end
function c95000160.op(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=Duel.GetFieldCard(tp,LOCATION_SZONE,5)
	local tc2=Duel.GetFieldCard(1-tp,LOCATION_SZONE,5)	
	
	--check if number of card >20 if speed duel or >40 if other duel
	if Duel.IsExistingMatchingCard(c95000160.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) and Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,nil):GetCount()<20 then
	Duel.Win(1-tp,0x55)
	end
	if Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,e:GetHandler()):GetCount()<40 and not Duel.IsExistingMatchingCard(c95000160.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
	Duel.Win(1-tp,0x55)
	end
	...
	if tc==nil then
		...
		if tc2==nil then
			local token=Duel.CreateToken(tp,95000160,nil,nil,nil,nil,nil,nil)		
		...
		-- if Duel.IsExistingMatchingCard(c95000160.Vfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
			-- c95000160.tableAction.push(95000200)
		-- end
	else
...
-- Add Action Card
function c95000160.Acttarget(e,tp,eg,ep,ev,re,r,rp,chk)
...
Duel.RegisterFlagEffect(tp,95000160,RESET_PHASE+PHASE_END,0,1)
end
if dc==1 or dc==2 then
if not Duel.IsExistingMatchingCard(c95000160.cfilter,tp,LOCATION_SZONE+LOCATION_HAND,0,1,nil) then	
...
if dc==5 or dc==6 then
 if not Duel.IsExistingMatchingCard(c95000160.cfilter,1-tp,LOCATION_SZONE+LOCATION_HAND,0,1,nil) then
...
function c95000160.condition(e,tp,eg,ep,ev,re,r,rp)
	...
	and Duel.GetFlagEffect(e:GetHandlerPlayer(),95000160)==0
	and not e:GetHandler():IsStatus(STATUS_CHAINING)
end
function c95000160.cfilter(c)
...
function c95000160.repop(e)
	local c=e:GetHandler()
		...
		e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		e1:SetLabelObject(c)
		e1:SetOperation(c95000160.returnop)
		Duel.RegisterEffect(e1,0)
		c:RegisterFlagEffect(900000007,0,0,1)
	end
	Duel.SendtoDeck(c,nil,-2,REASON_RULE)
end
function c95000160.returnop(e)
	...
	if fc and fc:GetFlagEffect(195000160)==0 then
	--action card get
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(95000160,0))
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetRange(LOCATION_SZONE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCondition(c95000160.condition)
	e1:SetTarget(c95000160.Acttarget)
