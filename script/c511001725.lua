...
function c511001725.initial_effect(c)
	--spsummon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(511001725,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_HAND)
	e1:SetCost(c511001725.spcost)
	e1:SetTarget(c511001725.sptg)
	e1:SetOperation(c511001725.spop)
	c:RegisterEffect(e1)
	c511001725.spe=e1
	if not c511001725.global_check then
		c511001725.global_check=true
		local ge2=Effect.CreateEffect(c)
		ge2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge2:SetCode(EVENT_ADJUST)
		ge2:SetCountLimit(1)
		ge2:SetProperty(EFFECT_FLAG_NO_TURN_RESET)
		ge2:SetOperation(c511001725.cardchk)
		Duel.RegisterEffect(ge2,0)
	end
end
function c511001725.spcon(c,e)
	if c==nil or not e then return false end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and Duel.CheckReleaseGroup(tp,c511001725.filter,1,nil) 
...
function c511001725.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.SelectReleaseGroup(tp,c511001725.filter,1,1,nil)
...
function c511001725.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c511001725.cardchk(e,tp,eg,ep,ev,re,r,rp)
