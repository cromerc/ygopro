...
function c511009354.initial_effect(c)
	--
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e1:SetCode(EVENT_PRE_DAMAGE_CALCULATE)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCondition(c511009354.atkcon)
	e1:SetOperation(c511009354.atkop)
	c:RegisterEffect(e1)
	--def
	local e1=Effect.CreateEffect(c)
	...
	e1:SetTarget(c511009354.deftg)
	e1:SetOperation(c511009354.defop)
	c:RegisterEffect(e1)
end

function c511009354.atkcon(e,tp,eg,ep,ev,re,r,rp)
...
function c511009354.atkop(e,tp,eg,ep,ev,re,r,rp)
...
function c511009354.deftg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local tc=Duel.GetAttacker()
	if tc==c then tc=Duel.GetAttackTarget() end
	if chk==0 then return tc and tc:IsFaceup() end
end
function c511009354.defop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetAttacker()
	if tc==c then tc=Duel.GetAttackTarget() end
