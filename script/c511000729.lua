--Berserk Mode
function c511000729.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_POSITION)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c511000729.target)
	e1:SetOperation(c511000729.activate)
	c:RegisterEffect(e1)
end
function c511000729.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511000729.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(Card.IsDefensePos,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
	...
	while tc1 do
		...
		while tc2 do
			...
			if tc1:GetAttack()>tc2:GetAttack() then
				tc2:RegisterFlagEffect(511000729,RESET_CHAIN,0,1)
			...
			elseif tc1:GetAttack()<tc2:GetAttack() then
				tc1:RegisterFlagEffect(511000729,RESET_CHAIN,0,1)
	...
	local des=Duel.GetMatchingGroup(c511000729.desfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
	Duel.Destroy(des,REASON_BATTLE)
end
function c511000729.desfilter(c)
	return c:GetFlagEffect(511000729)>0
end
