--王家の眠る谷－ネクロバレー
function c511002998.initial_effect(c)
	--Activate
	...
	e4:SetCondition(c511002998.contp)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	e5:SetTargetRange(0,LOCATION_GRAVE)
	e5:SetCondition(c511002998.conntp)
	c:RegisterEffect(e5)
	--necro valley
	...
	e6:SetCondition(c511002998.contp)
	c:RegisterEffect(e6)
	local e7=e6:Clone()
	e7:SetTargetRange(0,LOCATION_GRAVE)
	e7:SetCondition(c511002998.conntp)
	c:RegisterEffect(e7)
	...
	e8:SetCondition(c511002998.contp)
	c:RegisterEffect(e8)
	local e9=e8:Clone()
	e9:SetTargetRange(0,1)
	e9:SetCondition(c511002998.conntp)
	c:RegisterEffect(e9)
	--disable
...
function c511002998.discheck(ev,category,re,im0,im1,targets)
	...
	if tg and tg:GetCount()>0 then
		if targets and targets:IsContains(re:GetHandler()) then
			return tg:IsExists(c511002998.disfilter,1,nil,im0,im1)
		else
			return tg:IsExists(c511002998.disfilter,1,re:GetHandler(),im0,im1)
		end
	end
	return false
end
function c511002998.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not res and c511002998.discheck(ev,CATEGORY_SPECIAL_SUMMON,re,im0,im1,targets) then res=true end
	if not res and c511002998.discheck(ev,CATEGORY_REMOVE,re,im0,im1,targets) then res=true end
	if not res and c511002998.discheck(ev,CATEGORY_TOHAND,re,im0,im1,targets) then res=true end
	if not res and c511002998.discheck(ev,CATEGORY_TODECK,re,im0,im1,targets) then res=true end
	if not res and c511002998.discheck(ev,CATEGORY_LEAVE_GRAVE,re,im0,im1,targets) then res=true end
