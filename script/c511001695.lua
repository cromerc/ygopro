...
function c511001695.initial_effect(c)
	...
	c:RegisterEffect(e1)
	c511001695.spe=e1
	if not c511001695.global_check then
		c511001695.global_check=true
		local ge2=Effect.CreateEffect(c)
		ge2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge2:SetCode(EVENT_ADJUST)
		ge2:SetCountLimit(1)
		ge2:SetProperty(EFFECT_FLAG_NO_TURN_RESET)
		ge2:SetOperation(c511001695.cardchk)
		Duel.RegisterEffect(ge2,0)
	end
end
function c511001695.spcon(c,e)
	if c==nil or not e then return false end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and Duel.CheckReleaseGroup(tp,c511001695.filter,1,nil) 
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c511001695.filter(c)
...
function c511001695.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c511001695.filter,1,nil) end
	local g=Duel.SelectReleaseGroup(tp,c511001695.filter,1,1,nil)
...
function c511001695.cardchk(e,tp,eg,ep,ev,re,r,rp)
