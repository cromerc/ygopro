--Commande Duel 9
function c95200009.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_HANDES+CATEGORY_TOHAND)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)	
	e1:SetTarget(c95200009.target)
	e1:SetOperation(c95200009.activate)
	c:RegisterEffect(e1)
end
function c95200009.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_HANDES,nil,0,PLAYER_ALL,1)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,PLAYER_ALL,LOCATION_GRAVE)
end
function c95200009.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if hg1:GetCount()>0 and hg2:GetCount()>0 then
