...
function c100000621.fcon(e,g,gc,chkfnf)
	...
	if gc then return (c100000621.filter1(gc,tp) and g:IsExists(c100000621.filter2,1,gc,tp))
		or (c100000621.filter2(gc,tp) and g:IsExists(c100000621.filter1,1,gc,tp)) end
	...
	while tc do
		...
		if tc:IsHasEffect(511002961) then b3=b3+1 match=true
			...
			if c100000621.filter1(tc,tp) then b1=1 match=true end
			if c100000621.filter2(tc,tp) then b2=1 match=true end
		end
		tc=g:GetNext()
	end
...
function c100000621.fop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	...
	if gc then
		...
		if c100000621.filter1(gc,tp) then sg:Merge(eg:Filter(c100000621.filter2,gc,tp)) end
		if c100000621.filter2(gc,tp) then sg:Merge(eg:Filter(c100000621.filter1,gc,tp)) end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
		local g1=sg:Select(tp,1,1,nil)
		Duel.SetFusionMaterial(g1)
		return
	end
	local g1=eg:Filter(c100000621.filter1,nil,tp)
	local g2=eg:Filter(c100000621.filter2,nil,tp)
