--Draw of Fate
function c513000109.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCategory(CATEGORY_DRAW)
	e1:SetCode(EVENT_ATTACK_ANNOUNCE)
	e1:SetCondition(c513000109.condition)
	e1:SetTarget(c513000109.target)
	e1:SetOperation(c513000109.activate)
	c:RegisterEffect(e1)
end
function c513000109.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetAttacker():IsControler(1-tp)
end
function c513000109.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) and Duel.IsPlayerCanDraw(1-tp,1) end
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,PLAYER_ALL,1)
end
function c513000109.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.NegateAttack() and Duel.Draw(1-tp,1,REASON_EFFECT)~=0 then
		...
		e1:SetOperation(c513000109.regop)
		e1:SetLabel(3)
		e1:SetLabelObject(g)
		...
		e2:SetOperation(c513000109.regop2)
		...
		e5:SetCountLimit(1)
		e5:SetOperation(c513000109.winop)
		e5:SetReset(RESET_PHASE+PHASE_END)
		e5:SetLabelObject(e1)
		Duel.RegisterEffect(e5,tp)
	end
end
function c513000109.regop(e,tp,eg,ep,ev,re,r,rp)
...
function c513000109.regop2(e,tp,eg,ep,ev,re,r,rp)
