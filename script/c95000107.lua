...
function c95000107.initial_effect(c)
	...
	e1:SetOperation(c95000107.op)
	c:RegisterEffect(e1)
	--redirect
	local e2=Effect.CreateEffect(c)
	e2:SetCode(EVENT_LEAVE_FIELD)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e2:SetOperation(c95000107.repop)
	c:RegisterEffect(e2)		
	--unaffectable
	...
	e8:SetCondition(c95000107.condition)
	e8:SetTarget(c95000107.Acttarget)
	e8:SetOperation(c95000107.operation)
	c:RegisterEffect(e8)
...
--speed Duel Filter
function c95000107.SDfilter(c)
	return c:GetCode()==511004001
end
--vanilla mode filter
function c95000107.Vfilter(c)
...
function c95000107.op(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.IsExistingMatchingCard(c95000107.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) and Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,nil):GetCount()<20 then
	Duel.Win(1-tp,0x55)
	end
	if Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,e:GetHandler()):GetCount()<40 and not Duel.IsExistingMatchingCard(c95000107.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
	Duel.Win(1-tp,0x55)
	end
	...
	if tc==nil then
		...
		-- if Duel.IsExistingMatchingCard(c95000107.Vfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
			-- c95000107.tableAction.push(95000200)
		-- end
	else
...
function c95000107.repop(e)
	local c=e:GetHandler()
		...
		e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		e1:SetLabelObject(c)
		e1:SetOperation(c95000107.returnop)
		Duel.RegisterEffect(e1,0)
		c:RegisterFlagEffect(900000007,0,0,1)
	end
	Duel.SendtoDeck(c,nil,-2,REASON_RULE)
end
function c95000107.returnop(e)
	...
	e1:SetCondition(c95000107.condition)
	e1:SetTarget(c95000107.Acttarget)
...
function c95000107.Fcond(e)
	local f1=Duel.GetFieldCard(0,LOCATION_SZONE,5)
	local f2=Duel.GetFieldCard(1,LOCATION_SZONE,5)
	return (f1:GetFlagEffect(195000107)==0) and (f2:GetFlagEffect(195000107)==0)
end
