...
function c100000351.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.CheckSpecialSummonActivity(tp) and not Duel.CheckSummonActivity(tp)
	 and Duel.GetFlagEffect(tp,100000351)==0 end
	--oath effects
	...
	Duel.RegisterFlagEffect(tp,100000351,RESET_PHASE+PHASE_END,EFFECT_FLAG_OATH,1)
end
...
function c100000351.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return not e:GetHandler():IsLocation(LOCATION_GRAVE) and Duel.IsExistingTarget(c100000351.filter,tp,LOCATION_MZONE,0,1,nil)
	 and Duel.IsExistingTarget(c100000351.filter2,tp,0,LOCATION_SZONE,1,nil) end
