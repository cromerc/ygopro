...
function c100214005.initial_effect(c)
	...
	e1:SetTarget(c100214005.damtg1)
	e1:SetOperation(c100214005.damop1)
	c:RegisterEffect(e1)
	...
	e2:SetCondition(c100214005.damcon2)
	e2:SetTarget(c100214005.damtg2)
	e2:SetOperation(c100214005.damop2)
	c:RegisterEffect(e2)
end
function c100214005.damtg1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(1-tp)
	Duel.SetTargetParam(1000)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,1000)
end
function c100214005.damop1(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Damage(p,d,REASON_EFFECT)
end
function c100214005.damcon2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return (c:IsReason(REASON_BATTLE) or (c:GetReasonPlayer()~=tp and c:IsReason(REASON_EFFECT)))
		and c:IsPreviousPosition(POS_FACEUP)
end
function c100214005.damfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xad)
end
function c100214005.damtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	local gc=Duel.GetMatchingGroupCount(c100214005.damfilter,tp,LOCATION_GRAVE,0,nil)
	if chk==0 then return gc>0 end
	Duel.SetTargetPlayer(1-tp)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,gc*500)
end
function c100214005.damop2(e,tp,eg,ep,ev,re,r,rp)
	local gc=Duel.GetMatchingGroupCount(c100214005.damfilter,tp,LOCATION_GRAVE,0,nil)
