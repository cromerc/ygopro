--Teletemporate
function c511002437.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_REMOVE)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c511002437.target)
	e1:SetOperation(c511002437.activate)
	c:RegisterEffect(e1)
end
function c511002437.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511002437.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if Duel.Remove(tc,tc:GetPosition(),REASON_EFFECT+REASON_TEMPORARY)>0 then
			...
			e1:SetCountLimit(1)
			e1:SetOperation(c511002437.retop)
			Duel.RegisterEffect(e1,tp)
		end
	end
end
function c511002437.retop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetTurnCount()~=e:GetLabel() and tp==Duel.GetTurnPlayer() then
		Duel.Hint(HINT_CARD,0,511002437)
