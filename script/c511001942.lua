--Bee Force - Arbalest the Quickshot
function c511001942.initial_effect(c)
	--special summon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(511001942,0))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e2:SetCode(EVENT_DESTROYED)
	e2:SetTarget(c511001942.sptg)
	e2:SetOperation(c511001942.spop)
	c:RegisterEffect(e2)
end
function c511001942.filter(c,e,tp)
	return c:IsCode(511001942) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
