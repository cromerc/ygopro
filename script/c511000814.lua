...
function c511000814.filter2(c,e,tp,m,f,chkf)
...
function c511000814.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local res=Duel.IsExistingMatchingCard(c511000814.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c511000814.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c511000814.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg1=Duel.GetMatchingGroup(c511000814.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c511000814.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
	end
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		local cg1=Duel.GetFieldGroup(tp,LOCATION_EXTRA,0)
