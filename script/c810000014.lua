...
function c810000014.initial_effect(c)
	--Activate
	...
	--e2:SetCondition(c810000014.condition)
	--e2:SetTarget(c810000014.target)
	e2:SetOperation(c810000014.activate)
	c:RegisterEffect(e2)
end
--[[function c810000014.condition(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsChainNegatable(ev) then return false end
	local ex=Duel.GetOperationInfo(ev,CATEGORY_DICE)
	return ex and re:GetHandler():GetControler(1-tp)
end
function c810000014.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_NEGATE,eg,1,0,0)
end]]--
function c810000014.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.SelectYesNo(tp,aux.Stringid(810000014,0)) then
		Duel.Hint(HINT_CARD,0,810000014)
