...
function c511009175.cfilter(c,e,tp)
	return c:IsSetCard(0xf0) and c:IsLevelBelow(4) and c:IsType(TYPE_MONSTER) and not c:IsPublic() 
		and Duel.IsExistingMatchingCard(c511009175.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,c:GetCode())
end
function c511009175.spfilter(c,e,tp,code)
	return c:IsCode(code) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c511009175.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c511009175.cfilter,tp,LOCATION_HAND,0,1,nil,e,tp) end
	e:SetLabel(0)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local g=Duel.SelectMatchingCard(tp,c511009175.cfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
