...
function c511009106.initial_effect(c)
	--spsummon
	...
	e1:SetCondition(c511009106.condition)
	e1:SetTarget(c511009106.target)
	e1:SetOperation(c511009106.operation)
	c:RegisterEffect(e1)
	--spsummon
	...
	if chk==0 then
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c511009106.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,c)
			end
		end
		return res
	end
...
function c511009106.fop(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg1=Duel.GetMatchingGroup(c511009106.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,c)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
