...
function c100000307.initial_effect(c)
	...
	e1:SetTarget(c100000307.sptarget)
	e1:SetOperation(c100000307.spactivate)
	c:RegisterEffect(e1)
...
function c100000307.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if not tc:IsStatus(STATUS_BATTLE_DESTROYED) and tc:GetLevel()>=5 and tc:IsPreviousPosition(POS_FACEUP)
 		and tc:IsPreviousLocation(LOCATION_MZONE) and tc:GetPreviousControler()==tp	then
			c100000307[tp]=false
		end
		tc=eg:GetNext()
	end
...
function c100000307.sptarget(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and not c100000307[tp]
		and Duel.IsExistingMatchingCard(c100000307.filter,tp,0x13,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,0x13)
end
function c100000307.spactivate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c100000307.filter,tp,0x13,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
	while tc do
