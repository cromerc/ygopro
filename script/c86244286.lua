--Power Balance
function c86244286.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_COIN+CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_ATTACK_ANNOUNCE)
	e1:SetCondition(c86244286.condition)
	e1:SetTarget(c86244286.target)
	e1:SetOperation(c86244286.activate)
	c:RegisterEffect(e1)
end
function c86244286.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c86244286.target(e,tp,eg,ep,ev,re,r,rp,chk)
if chk==0 then return Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)>0 end
Duel.SetOperationInfo(0,CATEGORY_HANDES,nil,0,1-tp,1)
Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,tp,0,1)
end
function c86244286.activate(e,tp,eg,ep,ev,re,r,rp)
