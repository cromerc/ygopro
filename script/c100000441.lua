...
function c100000441.cfilter(c,e,tp)
	return c:IsAbleToGraveAsCost() and (c:IsCode(100000053) or c:IsCode(100000047))
		and Duel.IsExistingMatchingCard(c100000441.spfilter,tp,0x13,0,1,nil,e,tp)
end
...
function c100000441.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1
			and Duel.IsExistingMatchingCard(c100000441.cfilter,tp,LOCATION_MZONE,0,1,nil,e,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c100000441.cfilter,tp,LOCATION_MZONE,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
