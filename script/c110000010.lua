...
function c110000010.initial_effect(c)
	...
	e8:SetOperation(c110000010.facechk)
	e8:SetLabelObject(e7)
	c:RegisterEffect(e8)
	--destroy
	local e9=Effect.CreateEffect(c)
	e9:SetDescription(aux.Stringid(110000010,1))
	...
	e9:SetCost(c110000010.descost)
	e9:SetTarget(c110000010.destg)
	e9:SetOperation(c110000010.desop)
	c:RegisterEffect(e9)
	--pay atk/def
	local e10=Effect.CreateEffect(c)
	e10:SetDescription(aux.Stringid(110000010,2))
	...
	e10:SetCost(c110000010.adcost)
	e10:SetOperation(c110000010.adop)
...
function c110000010.facechk(e,tp,eg,ep,ev,re,r,rp)
	e:GetLabelObject():SetLabel(1)
end
function c110000010.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckLPCost(tp,1000) end
	Duel.PayLPCost(tp,1000)
end
function c110000010.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c110000010.desop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc and tc:IsRelateToEffect(e) then
		Duel.Destroy(tc,REASON_EFFECT,LOCATION_REMOVED)
	end
end
function c110000010.adcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLP(tp)>1 end
	local lp=Duel.GetLP(tp)
	e:SetLabel(lp-1)
	Duel.PayLPCost(tp,lp-1)
end
function c110000010.adop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
