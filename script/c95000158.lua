--Action Field - Canyon
function c95000158.initial_effect(c)
--Activate	
	...
	e1:SetOperation(c95000158.op)
	c:RegisterEffect(e1)
	--redirect
	local e2=Effect.CreateEffect(c)
	e2:SetCode(EVENT_LEAVE_FIELD)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e2:SetOperation(c95000158.repop)
	c:RegisterEffect(e2)		
	--unaffectable
	...
	e8:SetCondition(c95000158.condition)
...
--speed Duel Filter
function c95000158.SDfilter(c)
	return c:GetCode()==511004001
end
--vanilla mode filter
function c95000158.Vfilter(c)
	return c:GetCode()==511004002
end
function c95000158.op(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=Duel.GetFieldCard(tp,LOCATION_SZONE,5)
	local tc2=Duel.GetFieldCard(1-tp,LOCATION_SZONE,5)	
	
	--check if number of card >20 if speed duel or >40 if other duel
	if Duel.IsExistingMatchingCard(c95000158.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) and Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,nil):GetCount()<20 then
	Duel.Win(1-tp,0x55)
	end
	if Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,e:GetHandler()):GetCount()<40 and not Duel.IsExistingMatchingCard(c95000158.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
	Duel.Win(1-tp,0x55)
	end
	...
	if tc==nil then
		...
		-- if Duel.IsExistingMatchingCard(c95000158.Vfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
			-- c95000158.tableAction.push(95000200)
		-- end
	else
...
-- Add Action Card
function c95000158.Acttarget(e,tp,eg,ep,ev,re,r,rp,chk)
...
Duel.RegisterFlagEffect(tp,95000158,RESET_PHASE+PHASE_END,0,1)
end
...
if dc==5 or dc==6 then
 if not Duel.IsExistingMatchingCard(c95000158.cfilter,1-tp,LOCATION_SZONE+LOCATION_HAND,0,1,nil) then
...
function c95000158.condition(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(c95000158.cfilter,tp,LOCATION_SZONE+LOCATION_HAND,0,1,nil)
	and Duel.GetFlagEffect(e:GetHandlerPlayer(),95000158)==0
...
function c95000158.repop(e)
	local c=e:GetHandler()
		...
		e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		e1:SetLabelObject(c)
		e1:SetOperation(c95000158.returnop)
		Duel.RegisterEffect(e1,0)
		c:RegisterFlagEffect(900000007,0,0,1)
	end
	Duel.SendtoDeck(c,nil,-2,REASON_RULE)
end
function c95000158.returnop(e)
	...
	if fc and fc:GetFlagEffect(195000158)==0 then
	--action card get
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(95000158,0))
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetRange(LOCATION_SZONE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCondition(c95000158.condition)
	e1:SetTarget(c95000158.Acttarget)
...
function c95000158.IRLcondition(e,tp,eg,ep,ev,re,r,rp)
