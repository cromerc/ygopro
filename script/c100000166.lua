...
function c100000166.costfilter(c)
	return c:IsAttribute(ATTRIBUTE_WATER)
end
function c100000166.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c100000166.costfilter,1,nil) end
	local g=Duel.SelectReleaseGroup(tp,c100000166.costfilter,1,1,nil)
...
function c100000166.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if conf:GetCount()>0 then
		...
		local dg=conf:Filter(c100000166.filter,nil)
...
function c100000166.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	local dg=hg:Filter(c100000166.filter,nil)
	Duel.Destroy(dg,REASON_EFFECT)
	Duel.Damage(ep,dg:GetCount()*500,REASON_EFFECT)
	Duel.ShuffleHand(ep)
end
