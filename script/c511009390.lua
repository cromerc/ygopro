...
function c511009390.ffilter2(c)
	return c:IsAttribute(ATTRIBUTE_EARTH) and (c:GetLevel()==5 or c:GetLevel()==6)
end
--------------
...
function c511009390.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c511009390.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
end
...
function c511009390.ffilter(c,e,tp)
	return c:IsCanBeFusionMaterial() and c:IsAttribute(ATTRIBUTE_EARTH) and (c:GetLevel()==5 or c:GetLevel()==6)
end
