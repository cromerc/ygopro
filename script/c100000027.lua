...
function c100000027.filter2(c,tp)
	return ((c:GetLevel()==5 or c:GetLevel()==6) and c:IsLocation(LOCATION_GRAVE) and c:IsControler(1-tp)) or c:IsHasEffect(511002961)
end
function c100000027.fcon(e,g,gc,chkfnf)
	...
	while tc do
		...
		if tc:IsHasEffect(511002961) then b3=b3+1 match=true
			...
			if c100000027.filter1(tc,tp) then b1=1 match=true end
			if c100000027.filter2(tc,tp) then b2=1 match=true end
		end
