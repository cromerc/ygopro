...
function c95178994.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(95178994,1)) then
		Duel.BreakEffect()
		local gc=g:GetFirst()
		Duel.SpecialSummonStep(gc,0,tp,tp,false,false,POS_FACEUP_ATTACK)
		ft=ft-1
		gc=g:GetNext()
		while ft>0 and gc and not Duel.IsPlayerAffectedByEffect(tp,59822133) and Duel.SelectYesNo(tp,aux.Stringid(95178994,1)) do
			Duel.SpecialSummonStep(gc,0,tp,tp,false,false,POS_FACEUP_ATTACK)
			ft=ft-1
			gc=g:GetNext()
		end
