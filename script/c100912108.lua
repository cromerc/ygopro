...
function c100912108.initial_effect(c)
	--activate
	...
	e1:SetCountLimit(1,100912108+EFFECT_COUNT_CODE_OATH)
	e1:SetTarget(c100912108.target)
	e1:SetOperation(c100912108.activate)
	c:RegisterEffect(e1)
	--act in hand
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_TRAP_ACT_IN_HAND)
	e2:SetCondition(c100912108.handcon)
	c:RegisterEffect(e2)
end
function c100912108.filter(c)
...
function c100912108.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetChainInfo(0,CHAININFO_TARGET_CARDS):Filter(Card.IsRelateToEffect,nil,e)
	if g:GetCount()>0 then
		Duel.Destroy(g,REASON_EFFECT)
	end
end
function c100912108.handcon(e)
