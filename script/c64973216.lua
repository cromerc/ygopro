...
function c64973216.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
    e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_PLAYER_TARGET+EFFECT_FLAG_DAMAGE_STEP)
	e1:SetOperation(c64973216.activate)
	c:RegisterEffect(e1)
end
function c64973216.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	e2:SetCondition(c64973216.skipcon)
	...
	if Duel.GetTurnPlayer()==tp and Duel.GetCurrentPhase()==PHASE_BATTLE then
		e5:SetCondition(c64973216.skipcon)
