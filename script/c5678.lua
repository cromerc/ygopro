--ゴゴゴアリステラ＆デクシア
function c5678.initial_effect(c)
	--untargetable
	...
	e1:SetCondition(c5678.tgcon)
	e1:SetValue(c5678.atlimit)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	e2:SetCode(EFFECT_CANNOT_SELECT_EFFECT_TARGET)
	e2:SetProperty(EFFECT_FLAG_SET_AVAILABLE)
	e2:SetTargetRange(0,0xff)
	e2:SetValue(c5678.tglimit)
	c:RegisterEffect(e2)
	--effect gain
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e3:SetCode(EVENT_BE_MATERIAL)
	e3:SetCondition(c5678.effcon)
	e3:SetOperation(c5678.effop)
	c:RegisterEffect(e3)
end
function c5678.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x59)
end
function c5678.tgcon(e)
	return Duel.IsExistingMatchingCard(c5678.cfilter,e:GetHandlerPlayer(),LOCATION_MZONE,LOCATION_MZONE,1,e:GetHandler())
end
function c5678.atlimit(e,c)
	return c:IsFaceup() and c:IsSetCard(0x59)
end
function c5678.tglimit(e,re,c)
	return c:IsFaceup() and c:IsSetCard(0x59) and c:IsType(TYPE_MONSTER)
end
function c5678.effcon(e,tp,eg,ep,ev,re,r,rp)
	local mg=e:GetHandler():GetReasonCard():GetMaterial()
	return r==REASON_XYZ and mg:IsExists(Card.IsSetCard,mg:GetCount(),nil,0x59)
end
function c5678.effop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,5678)
	local c=e:GetHandler()
	local rc=c:GetReasonCard()
	local e1=Effect.CreateEffect(rc)
	e1:SetDescription(aux.Stringid(5678,0))
	e1:SetCategory(CATEGORY_POSITION+CATEGORY_DEFCHANGE)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetCode(EVENT_SPSUMMON_SUCCESS)
	e1:SetCondition(c5678.poscon)
	e1:SetTarget(c5678.postg)
	e1:SetOperation(c5678.posop)
...
function c5678.poscon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_XYZ
end
function c5678.postg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c5678.posop(e,tp,eg,ep,ev,re,r,rp)
