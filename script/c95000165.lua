...
function c95000165.initial_effect(c)
	...
	e2:SetOperation(c95000165.repop)
	c:RegisterEffect(e2)		
	--unaffectable
	...
	e8:SetDescription(aux.Stringid(95000165,0))
	e8:SetType(EFFECT_TYPE_QUICK_O)
	e8:SetRange(LOCATION_SZONE)
	e8:SetCode(EVENT_FREE_CHAIN)
	e8:SetCondition(c95000165.condition)
	e8:SetTarget(c95000165.Acttarget)
	e8:SetOperation(c95000165.operation)
	c:RegisterEffect(e8)
	--cost change
	local e9=Effect.CreateEffect(c)
	e9:SetType(EFFECT_TYPE_FIELD)
	e9:SetCode(EFFECT_LPCOST_CHANGE)
	e9:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e9:SetRange(LOCATION_FZONE)
	e9:SetTargetRange(1,1)
	e9:SetCondition(c95000165.costchange)
	c:RegisterEffect(e9)
	--search
	...
	e10:SetOperation(c95000165.regop)
	e10:SetCondition(c95000165.IRLcondition)
	c:RegisterEffect(e10)
	local e11=Effect.CreateEffect(c)
	e11:SetDescription(aux.Stringid(94585852,0))
	e11:SetCategory(CATEGORY_TOHAND)
	e11:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e11:SetProperty(EFFECT_FLAG_EVENT_PLAYER+EFFECT_FLAG_DAMAGE_STEP)
	e11:SetCode(95000165)
	e11:SetTarget(c95000165.target)
	e11:SetCondition(c95000165.IRLcondition)
	e11:SetOperation(c95000165.operation)
...
--speed Duel Filter
function c95000165.SDfilter(c)
	return c:GetCode()==511004001
end
--vanilla mode filter
function c95000165.Vfilter(c)
...
function c95000165.op(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.IsExistingMatchingCard(c95000165.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) and Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,nil):GetCount()<20 then
	Duel.Win(1-tp,0x55)
	end
	if Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,e:GetHandler()):GetCount()<40 and not Duel.IsExistingMatchingCard(c95000165.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
	Duel.Win(1-tp,0x55)
	end
	...
	if tc==nil then
		...
		-- if Duel.IsExistingMatchingCard(c95000165.Vfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
			-- c95000165.tableAction.push(95000200)
		-- end
	else
...
Duel.RegisterFlagEffect(tp,95000165,RESET_PHASE+PHASE_END,0,1)
end
...
function c95000165.condition(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(c95000165.cfilter,tp,LOCATION_SZONE+LOCATION_HAND,0,1,nil)
	and Duel.GetFlagEffect(e:GetHandlerPlayer(),95000165)==0
...
function c95000165.repop(e)
	local c=e:GetHandler()
		...
		e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		e1:SetLabelObject(c)
		e1:SetOperation(c95000165.returnop)
		Duel.RegisterEffect(e1,0)
		c:RegisterFlagEffect(900000007,0,0,1)
	end
	Duel.SendtoDeck(c,nil,-2,REASON_RULE)
end
function c95000165.returnop(e)
	...
	if fc and fc:GetFlagEffect(195000165)==0 then
	--action card get
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(95000165,0))
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetRange(LOCATION_SZONE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCondition(c95000165.condition)
	e1:SetTarget(c95000165.Acttarget)
	e1:SetOperation(c95000165.operation)
	...
	fc:RegisterFlagEffect(195000165,RESET_EVENT+0x1fe0000,0,1)
	end
end
