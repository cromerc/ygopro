...
function c511001606.initial_effect(c)
	...
	e1:SetOperation(c511001606.operation)
	c:RegisterEffect(e1)
end
function c511001606.operation(e,tp,eg,ep,ev,re,r,rp)
	--spsummon
	local ec1=Effect.CreateEffect(e:GetHandler())
	ec1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	ec1:SetCode(EVENT_SUMMON_SUCCESS)
	ec1:SetCondition(c511001606.check)
	ec1:SetLabel(0)
	ec1:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(ec1,tp)
	local ec2=ec1:Clone()
	ec2:SetCode(EVENT_SPSUMMON_SUCCESS)
	ec2:SetCondition(c511001606.check2)
	...
	e2:SetCondition(c511001606.spcon)
	e2:SetOperation(c511001606.spop)
	e2:SetLabelObject(ec1)
	e2:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e2,tp)
	local e3=e2:Clone()
	e3:SetCode(EVENT_SPSUMMON_SUCCESS)
	e3:SetCondition(c511001606.spcon2)
	...
	e8:SetCondition(c511001606.spcon)
	e8:SetOperation(c511001606.regop)
...
function c511001606.check(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==2 then return end
	if eg:IsExists(c511001606.cfilter,1,nil,tp) then
		e:SetLabel(1)
	end
end
function c511001606.check2(e,tp,eg,ep,ev,re,r,rp)
...
function c511001606.spcon2(e,tp,eg,ep,ev,re,r,rp)
	return e:GetLabelObject():GetLabel()==1 and Duel.GetCurrentChain()==0
end
function c511001606.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c511001606.regop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabelObject():GetLabel()~=1 then return end
	...
	e1:SetCondition(c511001606.spcon3)
	e1:SetOperation(c511001606.spop3)
	e1:SetReset(RESET_EVENT+EVENT_ADJUST)
	tc:RegisterEffect(e1)
end
function c511001606.spcon3(e,c,og)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and e:GetLabelObject():GetLabel()==1
		and Duel.IsExistingMatchingCard(c511001606.spfilter,tp,LOCATION_HAND,0,1,nil,e,tp)
end
function c511001606.spop3(e,tp,eg,ep,ev,re,r,rp,c,og)
	Duel.Hint(HINT_CARD,0,511001606)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c511001606.spfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
	og:Merge(g)
