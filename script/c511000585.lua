--Level Jar
function c511000585.initial_effect(c)
	--flip
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_FLIP)
	e1:SetOperation(c511000585.operation)
	c:RegisterEffect(e1)
end
function c511000585.filter(c)
	return not c:IsStatus(STATUS_BATTLE_DESTROYED) and c:IsAbleToDeck()
end
function c511000585.operation(e,tp,eg,ep,ev,re,r,rp)
	local rg1=Duel.GetMatchingGroup(c511000585.filter,tp,LOCATION_MZONE,0,nil)
	Duel.SendtoDeck(rg1,nil,2,REASON_EFFECT)
	rg1=rg1:Filter(Card.IsLocation,nil,LOCATION_DECK+LOCATION_EXTRA)
	local rg2=Duel.GetMatchingGroup(c511000585.filter,tp,0,LOCATION_MZONE,nil)
