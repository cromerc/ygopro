...
function c100000156.synfilter1(c,lv,tuner,syncard)
	local tlv=c:GetSynchroLevel(syncard)
	if c:GetFlagEffect(100000147)==0 then	
		return tuner:IsExists(c100000156.synfilter2,1,nil,lv+tlv,syncard)
	else
		return tuner:IsExists(c100000156.synfilter2,1,nil,lv-tlv,syncard)
	end	
end
function c100000156.synfilter2(c,lv,syncard)
...
function c100000156.syncon(e,c,tuner)
	...
	local tuner=Duel.GetMatchingGroup(c100000156.tmatfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)
	local nontuner=Duel.GetMatchingGroup(c100000156.ntmatfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)
	return nontuner:IsExists(c100000156.synfilter1,1,nil,4,tuner,c)
end
function c100000156.synop(e,tp,eg,ep,ev,re,r,rp,c,tuner)
	local g=Group.CreateGroup()
	local tun=Duel.GetMatchingGroup(c100000156.tmatfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)
	local nont=Duel.GetMatchingGroup(c100000156.ntmatfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SMATERIAL)
	local nontmat=nont:FilterSelect(tp,c100000156.synfilter1,1,1,nil,4,tun,c)
	...
	if mat1:GetFlagEffect(100000147)==0 then
		t=tun:FilterSelect(tp,c100000156.synfilter2,1,1,nil,4+lv1,c)
	else
		t=tun:FilterSelect(tp,c100000156.synfilter2,1,1,nil,4-lv1,c)
	end
	g:Merge(t)
