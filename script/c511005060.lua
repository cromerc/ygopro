...
function scard.flag_op(e,tp,eg,ep,ev,re,r,rp)
  ...
  scard['cstore_'..ch]={eg,ep,ev,re,r,rp}
end

function scard.hnd_fil(c,e,tp,eg,ep,ev,re,r,rp)
  if c:IsType(TYPE_SPELL) and not scard['no_react_ev'] then
...
function scard.szo_fil(c,e,tp,eg,ep,ev,re,r,rp)
  return not c:IsFaceup() and scard.hnd_fil(c,e,tp,eg,ep,ev,re,r,rp)
end
...
function scard.tg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then
    ...
    if loc>0 then
      return Duel.IsExistingMatchingCard(scard.szo_fil,tp,LOCATION_SZONE,0,1,e:GetHandler(),e,tp,eg,ep,ev,re,r,rp) or Duel.IsExistingMatchingCard(scard.hnd_fil,tp,LOCATION_HAND,0,1,e:GetHandler(),e,tp,eg,ep,ev,re,r,rp)
    elseif loc==0 then
      return Duel.IsExistingMatchingCard(scard.szo_fil,tp,LOCATION_SZONE,0,1,e:GetHandler(),e,tp,eg,ep,ev,re,r,rp)
    end
    return false
  end
  e:SetProperty(0)
...
function scard.op(e,tp,eg,ep,ev,re,r,rp)
  local og=Duel.GetMatchingGroup(scard.szo_fil,tp,LOCATION_SZONE,0,e:GetHandler(),e,tp,eg,ep,ev,re,r,rp)
  if Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
    og:Merge(Duel.GetMatchingGroup(scard.hnd_fil,tp,LOCATION_HAND,0,e:GetHandler(),e,tp,eg,ep,ev,re,r,rp))
  end
