...
function c95000028.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_NEGATE+CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_CHAINING)
	e1:SetCondition(c95000028.condition)
	e1:SetTarget(c95000028.target)
	e1:SetOperation(c95000028.activate)
	c:RegisterEffect(e1)
end
c95000028.mark=3
function c95000028.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c95000028.filter(c,tp,eg,ep,ev,re,r,rp)
...
function c95000028.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c95000028.activate(e,tp,eg,ep,ev,re,r,rp)
	Duel.NegateActivation(ev)
	if re:GetHandler():IsRelateToEffect(re) and Duel.Destroy(eg,REASON_EFFECT)>0 then
		local sg=Duel.GetMatchingGroup(c95000028.filter,tp,0,LOCATION_DECK,nil,tp,eg,ep,ev,re,r,rp)
