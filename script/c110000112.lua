...
function c110000112.initial_effect(c)
	...
	c:RegisterEffect(e5)
	if not c110000112.global_check then
		c110000112.global_check=true
		if Duel.GetFlagEffect(0,110000104)==0 and Duel.GetFlagEffect(1,110000104)==0 then
			...
			ge1:SetCondition(c110000112.atkcon)
			ge1:SetTarget(c110000112.atktg)
			...
			ge2:SetOperation(c110000112.checkop)
			...
			ge3:SetCondition(c110000112.atcon)
			ge3:SetOperation(c110000112.atop)
...
function c110000112.atcon(e,tp,eg,ep,ev,re,r,rp)
	local at=Duel.GetAttackTarget()
	return at:IsFaceup() and at:IsControler(tp) and at:IsType(0x10000000)
end
function c110000112.filter(c)
	return c:IsFaceup() and c:IsType(0x10000000)
end
function c110000112.atop(e,tp,eg,ep,ev,re,r,rp)
	local atg=Duel.GetAttacker():GetAttackableTarget()
	local d=Duel.GetAttackTarget()
	if atg:IsExists(c110000112.filter,1,d) and Duel.SelectYesNo(tp,aux.Stringid(21558682,0)) then
		local g=atg:FilterSelect(tp,c110000112.filter,1,1,d)
