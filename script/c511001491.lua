--Overlay Burglary
function c511001491.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_BE_BATTLE_TARGET)
	e1:SetCondition(c511001491.condition)
	e1:SetOperation(c511001491.activate)
	c:RegisterEffect(e1)
end
function c511001491.condition(e,tp,eg,ep,ev,re,r,rp)
	local a=Duel.GetAttacker()
	local at=Duel.GetAttackTarget()
	return at:IsControler(tp) and at:IsFaceup() and at:IsType(TYPE_XYZ) and a:IsType(TYPE_XYZ) and a:GetOverlayCount()>0
end
function c511001491.activate(e,tp,eg,ep,ev,re,r,rp)
	local a=Duel.GetAttacker()
	local at=Duel.GetAttackTarget()
