...
function c100000007.initial_effect(c)
	...
	c:RegisterEffect(e4)
	local e5=e3:Clone()
	e5:SetCondition(c100000007.damcon)
	...
	e6:SetCondition(c100000007.condition)
	e6:SetTarget(c100000007.target)
	e6:SetOperation(c100000007.activate)
	c:RegisterEffect(e6)
	--to deck
...
function c100000007.damcon(e)
	return e:GetHandler():IsAttackPos()
end
function c100000007.filter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToDeck() 
end
function c100000007.condition(e)
	local c=e:GetHandler()
	return e:GetHandler():GetBattledGroupCount()>0 and 
	Duel.IsExistingMatchingCard(c100000007.filter,c:GetControler(),0,LOCATION_SZONE,1,nil)
end
function c100000007.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(c100000007.filter,tp,0,LOCATION_ONFIELD,1,c) end
	local sg=Duel.GetMatchingGroup(c100000007.filter,tp,0,LOCATION_ONFIELD,c)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,sg,sg:GetCount(),0,0)
end
function c100000007.activate(e,tp,eg,ep,ev,re,r,rp)
	local sg=Duel.GetMatchingGroup(c100000007.filter,tp,0,LOCATION_ONFIELD,e:GetHandler())
