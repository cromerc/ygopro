--死なばもろとも
function c511000627.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c511000627.target)
	e1:SetOperation(c511000627.activate)
	c:RegisterEffect(e1)
end
function c511000627.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
...
function c511000627.activate(e,tp,eg,ep,ev,re,r,rp)
	local h1=Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)
	local h2=Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)
