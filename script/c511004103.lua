...
function c511004103.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.GetLocationCount(tp,LOCATION_MZONE,0)>0 
		and Duel.IsExistingTarget(c511004103.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp)end
...
function c511004103.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)>0 then
		if c:IsRelateToEffect(e) then
			...
			e1:SetCost(c511004103.drcost)
			e1:SetTarget(c511004103.drtg)
			e1:SetOperation(c511004103.drop)
...
function c511004103.drcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=e:GetLabelObject()
	if chk==0 then return tc and e:GetHandler():GetOverlayGroup():IsContains(tc) end
	Duel.SendtoGrave(tc,REASON_COST)
end
function c511004103.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,2) end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(2)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,2)
end
function c511004103.drop(e,tp,eg,ep,ev,re,r,rp,chk)
