...
function c95000175.initial_effect(c)
	...
	e2:SetOperation(c95000175.repop)
	c:RegisterEffect(e2)		
	--unaffectable
	...
	e7:SetTarget(c95000175.sfilter)
	c:RegisterEffect(e7)
	-- Add Action Card
	local e8=Effect.CreateEffect(c)
	e8:SetDescription(aux.Stringid(95000175,0))
	e8:SetType(EFFECT_TYPE_QUICK_O)
	e8:SetRange(LOCATION_SZONE)
	e8:SetCode(EVENT_FREE_CHAIN)
	e8:SetCondition(c95000175.condition)
	e8:SetTarget(c95000175.Acttarget)
	e8:SetOperation(c95000175.operation)
	c:RegisterEffect(e8)
...
--speed Duel Filter
function c95000175.SDfilter(c)
	return c:GetCode()==511004001
end
--vanilla mode filter
function c95000175.Vfilter(c)
...
function c95000175.op(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.IsExistingMatchingCard(c95000175.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) and Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,nil):GetCount()<20 then
	Duel.Win(1-tp,0x55)
	end
	if Duel.GetMatchingGroup(nil,tp,LOCATION_HAND+LOCATION_DECK,0,e:GetHandler()):GetCount()<40 and not Duel.IsExistingMatchingCard(c95000175.SDfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
	Duel.Win(1-tp,0x55)
	end
	...
	if tc==nil then
		...
		-- if Duel.IsExistingMatchingCard(c95000175.Vfilter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_REMOVED,0,1,nil) then
			-- c95000175.tableAction.push(95000200)
		-- end
	else
...
function c95000175.sfilter(e,c,tp)
...
Duel.RegisterFlagEffect(tp,95000175,RESET_PHASE+PHASE_END,0,1)
end
...
function c95000175.condition(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(c95000175.cfilter,tp,LOCATION_SZONE+LOCATION_HAND,0,1,nil)
	and Duel.GetFlagEffect(e:GetHandlerPlayer(),95000175)==0
...
function c95000175.repop(e)
	local c=e:GetHandler()
		...
		e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		e1:SetLabelObject(c)
		e1:SetOperation(c95000175.returnop)
		Duel.RegisterEffect(e1,0)
		c:RegisterFlagEffect(900000007,0,0,1)
	end
	Duel.SendtoDeck(c,nil,-2,REASON_RULE)
end
function c95000175.returnop(e)
	...
	if fc and fc:GetFlagEffect(195000175)==0 then
	--action card get
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(95000175,0))
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetRange(LOCATION_SZONE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCondition(c95000175.condition)
	e1:SetTarget(c95000175.Acttarget)
	e1:SetOperation(c95000175.operation)
	...
	e4:SetTarget(c95000175.sfilter)
	fc:RegisterEffect(e4)
	fc:RegisterFlagEffect(195000175,RESET_EVENT+0x1fe0000,0,1)
	end
end
