...
function c511001101.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(511001101,0))
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_SUMMON_SUCCESS)
	e2:SetProperty(EFFECT_FLAG_DAMAGE_STEP)
	e2:SetTarget(c511001101.placetg)
	e2:SetOperation(c511001101.placeop)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e5:SetDescription(aux.Stringid(511001101,1))
	e5:SetCode(EFFECT_SEND_REPLACE)
	e5:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e5:SetProperty(EFFECT_FLAG_SINGLE_RANGE+EFFECT_FLAG_UNCOPYABLE)
	e5:SetRange(LOCATION_MZONE)
	e5:SetTarget(c511001101.crystaltg)
	e5:SetOperation(c511001101.crystalop)
	c:RegisterEffect(e5)
	--selfdes
...
function c511001101.crystaltg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511001101.crystalop(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c511001101.placetg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsSetCard,tp,0x33,0,1,nil,0x1034)
		and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 end
end
function c511001101.placeop(e,tp,eg,ep,ev,re,r,rp,chk)
