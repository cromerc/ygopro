--機皇神龍アステリスク
function c511001644.initial_effect(c)
	--special summon
	...
	e1:SetCondition(c511001644.spcon)
	c:RegisterEffect(e1)
	--attack
	...
	e3:SetCondition(c511001644.damcon)
	e3:SetTarget(c511001644.damtg)
	e3:SetOperation(c511001644.damop)
	c:RegisterEffect(e3)
	--cannot be target
	...
	e5:SetTarget(c511001644.desreptg)
	c:RegisterEffect(e5)
end
function c511001644.spfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x13)
end
function c511001644.spcon(e,c)
	if c==nil then return true end
	return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c511001644.spfilter,c:GetControler(),LOCATION_MZONE,0,3,nil)
end
function c511001644.atkfilter(c)
	return c:IsPosition(POS_FACEUP_ATTACK) and c:IsRace(RACE_MACHINE)
end
function c511001644.atkval(e,c)
	local g=Duel.GetMatchingGroup(c511001644.atkfilter,c:GetControler(),LOCATION_MZONE,0,c)
	return g:GetSum(Card.GetAttack)
end
function c511001644.damcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(Card.IsType,1,nil,TYPE_SYNCHRO)
end
function c511001644.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c511001644.damop(e,tp,eg,ep,ev,re,r,rp)
...
function c511001644.tg(e,c)
	return c:IsFaceup() and c:IsRace(RACE_MACHINE) and c~=e:GetHandler()
end
function c511001644.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
